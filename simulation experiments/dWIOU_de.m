...
dx1=(A(1)-B(1))*(A(1)-B(1));    % The sum of the squares of x
dy1=(A(2)-B(2))*(A(2)-B(2));    % The sum of the squares of y
dx1_loss=exp(dx1/(CCw*CCw));
dy1_loss=exp(dy1/(CCh*CCh)); 
d_loss=exp(d/C);
s.dx = -1*dx1_loss*(2*(A(1)-B(1))/(CCw*CCw))*d_loss;
s.dy = -1*dy1_loss*(2*(A(2)-B(2))/(CCh*CCh))*d_loss;
s.dw = 0;
s.dh = 0;
...
% Derivative
s.dx=(p.dx*d_loss)+((1-iou_value)*s.dx);
s.dy=(p.dy*d_loss)+((1-iou_value)*s.dy);
s.dw=p.dw+s.dw;
s.dh=p.dh+s.dh;
